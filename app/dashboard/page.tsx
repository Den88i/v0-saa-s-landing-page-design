...
export default function HomePage() {
  ...
  const handleLogout = () => {
    // Aquí iría la lógica real de cierre de sesión (e.g., limpiar token, redirigir)
    // Por ahora, solo mostramos la notificación.
    toast.success("¡Sesión cerrada con éxito! Te esperamos de vuelta.", {
      duration: 3000, // Duración en milisegundos
      position: 'top-right', // Posición de la notificación
    });
    // Opcional: Redirigir al inicio o a la página de login después de un breve retraso
    // setTimeout(() => {
    //   window.location.href = '/login';
    // }, 1500);
  };

  const handleJoinTournament = (tournamentName: string, tournamentId: number) => {
    // Aquí iría la lógica real para unirse al torneo.
    // Podrías verificar si el usuario está logueado, tiene fondos, etc.
    // Por ahora, solo mostramos la notificación.

    // Ejemplo de validación básica:
    const isLoggedIn = false; // Simula si el usuario está logueado
    if (!isLoggedIn) {
      toast.error("Necesitas iniciar sesión para unirte a un torneo.", {
        duration: 4000,
    ...
    toast.success(`¡Te has unido a "${tournamentName}"! Buena suerte.`, {
      duration: 3000,
      position: 'top-center',
    });
    // Opcional: Redirigir a la página del torneo o confirmación
    // setTimeout(() => {
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center py-4">
              ...
              <div className="flex items-center space-x-4">
                {/* Modificado para usar handleLogout */}
                <Button variant="ghost" className="text-white hover:bg-white/10 border border-white/20" onClick={handleLogout}>
                  Cerrar Sesión
                </Button>
                <Link href="/register">
                ...
                return (
                  ...
                  >
                    ...
                    <CardContent className="space-y-4">
                      ...
                      {/* Modificado para usar handleJoinTournament */}
                      <Button
                        className="w-full bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-black font-semibold"
                        onClick={() => handleJoinTournament(safeName, safeId as number)}
                      >
                        Unirse al Torneo
                      </Button>
                    </CardContent>
            ...
            <div className="max-w-3xl mx-auto space-y-8">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <Link href="/how-to-play">
                  <Button
                    size="lg"
                    className="text-lg px-8 py-6 bg-green-500 text-white hover:bg-green-600"
                  >
                    Cómo Funciona
                  </Button>
                </Link>
